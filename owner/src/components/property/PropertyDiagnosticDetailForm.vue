...
<template>
  <NakedCard class="fr-mt-3w">
    ...
    <p>
      <label class="fr-label" for="rent">{{ t('propertydiagnostic.energy-consumption') }} <span
          style="color: red">*</span> :</label>
      <Field id="energy" name="energy" v-model="energyConsumption" v-slot="{ field, meta }" :rules="{
        required: true,
        positive: true,
      }">
        ...
        }" :placeholder="t('propertydiagnostic.energy-consumption-amount')" type="number" />
      </Field>
    ...
    <p>
      <label class="fr-label" for="charges">{{ t('propertydiagnostic.co2-emission') }} <span style="color: red">*</span>
        :</label>
      <Field id="co2" name="co2" v-model="co2Emission" v-slot="{ field, meta }" :rules="{
        required: true,
        positiveOrNull: true,
      }">
        <input v-bind="field" class="validate-required form-control fr-input" :class="{
          'fr-input--valid': meta.valid,
          'fr-input--error': !meta.valid,
        }" :placeholder="t('propertydiagnostic.co2-amount')" type="number" />
      </Field>
    ...
    <p>
      <label class="fr-label" for="charges">{{ t('propertydiagnostic.date-dpe') }} <span style="color: red">*</span>
        :</label>
      <Field id="dpeDate" name="dpeDate" v-model="dpeDate" v-slot="{ field, meta }" :rules="{
        required: true,
        positiveOrNull: true,
      }">
        <input v-bind="field" class="validate-required form-control fr-input" :class="{
          'fr-input--valid': meta.valid,
          'fr-input--error': !meta.valid,
        }" placeholder="13/12/2023" type="date" />
      </Field>
    ...
    <div class="fr-p-md-5w fr-mt-3w" v-if="localCo2Emission > 0 && localEnergyConsumption > 0">
      ...
      <div class="dpe-container">
        <DpeDiagram class="dpe-diagram" :letter="energyLetter" :consumption="localEnergyConsumption"></DpeDiagram>
        <DpeCo2Diagram class="dpe-diagram" :letter="co2Letter" :consumption="localCo2Emission"></DpeCo2Diagram>
      </div>
    </div>
    </Form>
  </NakedCard>
