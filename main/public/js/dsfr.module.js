...
const removeClass = (element, className) =>
  modifiyClass(element, className, true);
...
class Instance {
  ...
  unlisten(type, closure) {
    if (!type) {
      ...
      for (const closure of this.listeners[type])
  ...
  static getInstances(element, instanceClass) {
    if (!instances[element]) return null;
      ...
      return instances[element].filter(
        instance => instance instanceof instanceClass
      );
  }
}
...
class DisclosureButton {
  constructor(element, disclosure) {
    ...
    this.hasAttribute = this.element.hasAttribute(
      this.disclosure.attributeName
    );
...
class Disclosure extends Instance {
  constructor(element) {
    ...
    this.attributeName = this.type.ariaState
      ? "aria-" + this.type.id
      : ns.attr(this.type.id);
    this.pristine = true;
    ...
    if (buttons.length > 0)
      for (let i = 0; i < buttons.length; i++) this.addButton(buttons[i]);
  ...
  static build(from) {
    const elements = Array.prototype.slice.call(
      from.querySelectorAll(`.${this.selector}`)
    );
  ...
  conceal(withhold, preventFocus) {
    ...
    for (const disclosure of disclosures)
      if (disclosure !== this && this.element.contains(disclosure.element))
        disclosure.reset();
    return true;
  }
...
class KeyListener {
  ...
  down(key, closure, preventDefault, stopPropagation) {
    this._add(
      "down",
      new KeyAction(key, closure, preventDefault, stopPropagation)
    );
  }
      ...
      while (element) {
        if (element.classList.contains(ascendant)) {
          if (typeof ascendants[ascendant] === "string") {
            DisclosuresGroup.groupByParent(
              this,
              DisclosuresGroup,
...
const BREADCRUMB_COLLAPSE_SELECTOR = `${api.core.ns.selector(
  "breadcrumb"
)} ${api.core.ns.selector("collapse")}`;
...
class Breadcrumb extends api.core.Instance {
  ...
  resize() {
    if (window.matchMedia("(min-width: 48em)").matches) {
      if (this.collapse.buttons[0] === document.activeElement)
        this.links.focus();
    } else {
      if (this.links.indexOf(document.activeElement) > -1)
        this.collapse.focus();
    }
  }
}
...
const EQUISIZED_BUTTONS_GROUP_SELECTOR = api.core.ns.selector(
...
class FocusTrap {
  ...
  get focusables() {
     ...
     **/
    const radios = [
      ...document.documentElement.querySelectorAll('input[type="radio"]')
    ];
    ...
    const noDuplicates = unordereds.filter(
      element => ordereds.indexOf(element) === -1
    );
...
class Modal extends api.core.Disclosure {
  ...
  init() {
    ...
    this.keyListener.down(
      api.KeyListener.ESCAPE,
      this.conceal.bind(this),
      true,
      true
    );
  ...
  resize(isResizing, e) {
    ...
    if (this.body.scrollHeight > this.body.clientHeight) {
      if (
        this.body.offsetHeight + this.body.scrollTop >=
...
class NavigationMenu {
  ...
  place(right) {
    ...
    if (left + width > right)
      api.core.addClass(this.item, NAVIGATION_ITEM_RIGHT_CLASS);
...
class Navigation extends api.core.DisclosuresGroup {
  ...
  add(member) {
    ...
    if (member.element.classList.contains(NAVIGATION_MENU_CLASS)) {
      this.menus.push(
        new NavigationMenu(member, this.navList.getBoundingClientRect().right)
      );
    }
  }
...
class Scheme {
  ...
  mutate(mutations) {
    mutations.forEach(mutation => {
      if (
        mutation.type === "attributes" &&
        mutation.attributeName === SCHEME_ATTR
      ) {
...
const RADIOS_THEME_NAME = `input[name="${api.core.ns.selector(
  "radios-theme",
  ""
)}"]`;
...
const TABLE_SCROLLING_SELECTOR = `${api.core.ns.selector(
  "table"
)}:not(${api.core.ns.selector("table--no-scroll")})`;
const LEFT = "left";
const RIGHT = "right";
...
class Table {
  ...
  init(table) {
    ...
    this.isScrollable =
      this.tableContent.offsetWidth > this.tableElem.offsetWidth;
  ...
  change() {
    const newScroll =
      this.tableContent.offsetWidth > this.tableElem.offsetWidth;
    let firstTimeScrollable =
      this.tableElem.offsetWidth > this.table.offsetWidth;
  ...
  setShadowVisibility(side, scrollPosition) {
    ...
    if (scrollPosition <= SCROLL_OFFSET) {
      ...
      if (side === LEFT) api.core.addClass(this.table, SHADOW_LEFT_CLASS);
        ...
        api.core.addClass(this.table, SHADOW_RIGHT_CLASS);
    }
  }
}
...
const build$2 = () => {
  ...
  for (let i = 0; i < tableNodes.length; i++)
    tables.push(new Table(tableNodes[i]));
...
class TabsGroup extends api.core.DisclosuresGroup {
  ...
  init() {
    this.keyListener = new api.KeyListener(this.element);
    this.keyListener.down(
      api.KeyListener.RIGHT,
      this.arrowRightPress.bind(this),
      true,
      true
    );
    this.keyListener.down(
      api.KeyListener.LEFT,
      this.arrowLeftPress.bind(this),
      true,
      true
    );
    this.keyListener.down(
      api.KeyListener.HOME,
  ...
  render() {
    ...
    this.element.style.height =
      this.panelHeight + this.list.offsetHeight + "px";
  }
}
...
const HEADER_SEARCH_BAR_SELECTOR = `${HEADER_TOOLS_SELECTOR} ${api.core.ns.selector(
  "search-bar"
)}`;
const HEADER_SHORTCUTS_SELECTOR = `${HEADER_TOOLS_SELECTOR} ${api.core.ns.selector(
  "shortcuts"
)}`;
const HEADER_NAV_SELECTOR = api.core.ns.selector("nav");
const HEADER_NAV_LIST_SELECTOR = `${HEADER_NAV_SELECTOR} ${api.core.ns.selector(
  "nav__list"
)}`;
