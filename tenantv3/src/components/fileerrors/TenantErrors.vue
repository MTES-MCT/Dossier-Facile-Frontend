<template>
    ...
    >
      ...
      <div v-for="(v, k) in ['IDENTIFICATION', 'RESIDENCY', 'PROFESSIONAL', 'FINANCIAL', 'TAX']" :key="k">
        <div v-if="!isDocumentValid(v)">
              <UpdateComponent @on-update="openTenant(k+1)" :broken-rules="getDocumentBrokenRules(v)">{{ t(`fileerrors.${v}`) }}</UpdateComponent>
        </div>
      </div>
    </NakedCard>
...
const props = defineProps<{
  ...
  function getDocumentBrokenRules(docType: string) {
    return UtilsService.getTenantDocumentByType(props.user, docType)?.documentAnalysisReport?.brokenRules || [];
  }
