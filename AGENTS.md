...
- **Accessibilité (RGAA)** : axe prioritaire et obligation légale de viser un site "totalement conforme" au RGAA. 
  ...
  - **Liens d'évitement** : utiliser le composant partagé `SkipLinks.vue` / `DsfrSkipLinks` avec les constantes de `df-shared-next/src/models/SkipLink.ts`; chaque lien doit pointer vers un `id` réellement présent dans la page (`content`, `main-nav`, etc.).
