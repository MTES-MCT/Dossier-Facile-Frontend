...
- **Accessibilité (RGAA)** : axe prioritaire et obligation légale de viser un site 100% accessible. 
  - **Modales** : utiliser `DsfrModalPatch.vue` (a11y compatible) plutôt que le `DsfrModal` de `vue-dsfr`
  ...
  - **Contenu dynamique** : annoncer via `aria-live`. Vérifier la navigation clavier et le focus visible.
